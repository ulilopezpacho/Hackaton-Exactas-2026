...
function heldKarpOrder(
  ...
  for (let v = 0; v < n; v++) {
    const travel = getTravelTime(travelMatrix, startPlaceId ?? null, places[v].id);
...
function canScheduleDay(
  ...
  return findBestDaySchedule(places, dayIndex, day, mealPlaces, travelMatrix, config, [], startPlaceId) !== null;
}
...
function interDayLocalSearch(
  ...
  const { places, mealPlaces, days, travelMatrix, config, startPlaceId } = input;
  const numDays = days.length;

  let currentScore = computeSolutionScore(dayPlaceLists, travelMatrix, startPlaceId);
  let improved = true;
  let iteration = 0;
    ...
    for (let d1 = 0; d1 < numDays && !improved; d1++) {
      for (let pi = 0; pi < dayPlaceLists[d1].length && !improved; pi++) {
        ...
        for (let d2 = 0; d2 < numDays; d2++) {
          ...
          if (!canScheduleDay(candidateD2, d2, days[d2], mealPlaces, travelMatrix, config, d2 === 0 ? startPlaceId : null)) continue;

          const candidateD1 = dayPlaceLists[d1].filter((_, i) => i !== pi);
          if (!canScheduleDay(candidateD1, d1, days[d1], mealPlaces, travelMatrix, config, d1 === 0 ? startPlaceId : null)) continue;

          const saved = [dayPlaceLists[d1], dayPlaceLists[d2]];
          dayPlaceLists[d1] = candidateD1;
          dayPlaceLists[d2] = candidateD2;

          const candidateScore = computeSolutionScore(dayPlaceLists, travelMatrix, startPlaceId);
  ...
  for (let i = skipped.length - 1; i >= 0; i--) {
    ...
    for (let d = 0; d < numDays; d++) {
      ...
      if (!canScheduleDay(candidate, d, days[d], mealPlaces, travelMatrix, config, d === 0 ? startPlaceId : null)) continue;
...
function computeSolutionScore(
  ...
  for (let d = 0; d < dayPlaceLists.length; d++) {
    const dayList = dayPlaceLists[d];
    if (dayList.length < 2) continue;
    const ordered = nearestNeighborOrder(dayList, travelMatrix, d === 0 ? startPlaceId : null);
    let lastId: string | null = d === 0 ? (startPlaceId ?? null) : null;
    for (const p of ordered) {
...
function findBestDaySchedule(
  ...
  const ordered = heldKarpOrder(places, day, travelMatrix, config, startPlaceId)
